...
test("zero override is preserved and inputs reject invalid target and numeric values",()=>{
 ...
 const input={expectedRevision:0,requestKey:"test-request-123456",targetType:"overall",targetId:null,points:0};
 expect(scoreReviewInputSchema.safeParse(input).success).toBe(true);
 for(const delta of [{points:-1},{points:Infinity},{points:NaN},{targetId:"x"},{targetType:"item"},{reason:"x".repeat(1001)}])expect(scoreReviewInputSchema.safeParse({...input,...delta}).success).toBe(false);
});
