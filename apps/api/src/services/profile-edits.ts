...
import type { UpdatePatient, UpdateOrganizationUser } from "@niq/application-contracts";
  ...
  try {
    return await db.transaction(async tx => {
      ...
      const [updated] = await tx.update(patients).set({ homeFacilityId: facility.id, dateOfBirth: input.dateOfBirth, gender: input.gender,
        encryptedExternalReference: encryptPatientData(mrn, key), externalReferenceLookupHash: await keyedHash(mrn, key.toString("base64")),
      ...
      await audit(tx, actor, organizationId, context, "PATIENT_UPDATED", "patient", prior.id, Object.keys(input));
