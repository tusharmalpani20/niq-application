 ...
 async command(actor:Principal,org:string,id:string,raw:ClinicalReviewAction,context:RequestContext){
  ...
  return this.service.db.transaction(async tx=>{
   ...
   switch(input.action){
    ...
    case "COMPLETE":s.finalRemark=input.remark;s.completedAt=now.toISOString();s.finalSnapshot={score,scans,submissionId:row.currentSubmissionId,scoreRevision:projection.scoreRevision};completedAt=now;status="COMPLETED";break;
   }
