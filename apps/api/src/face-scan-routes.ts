...
async function json(c: any, maxBytes = 16 * 1024) {
  try {
    ...
    if (!(error instanceof SyntaxError))
      throw error;
    throw new ServiceError("VALIDATION_ERROR", "Invalid JSON request.");
  }
}
export function mountFaceScanRoutes(app: Hono<any>, service: AssessmentFaceScanService) {
  ...
  app.post(base, async (c) => c.json(await service.start(...parts(c), parse(startFaceScanSchema, await json(c)), {
