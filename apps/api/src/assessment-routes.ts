...
export function mountAssessmentRoutes(app:Hono<any>,service:AssessmentWorkflowService) {
  ...
  app.post(`${base}/assessment-initializations`,async c=>c.json(await service.initialize(c.get("principal"),parse(id,c.req.param("organizationId")),parse(initializeAssessmentSchema,await c.req.json()),{requestId:c.get("requestId")}),201));
  app.get(`${base}/assessment-initializations/:initializationId`,async c=>{const row=await service.getInitialization(c.get("principal"),parse(id,c.req.param("organizationId")),parse(id,c.req.param("initializationId")));return c.json({id:row.id,status:row.status,assessmentId:row.assessmentId,failureCode:row.failureCode});});
  ...
  app.get(`${base}/assessments/:assessmentId`,async c=>{const p=parts(c);return c.json(await service.read(p.actor,p.org,p.assessment));});
  app.patch(`${base}/assessments/:assessmentId`,async c=>{const p=parts(c);return c.json(await service.save(p.actor,p.org,p.assessment,parse(saveAssessmentSchema,await c.req.json()),p.context));});
  app.post(`${base}/assessments/:assessmentId/submit`,async c=>{const p=parts(c);return c.json(await service.submit(p.actor,p.org,p.assessment,parse(assessmentRevisionSchema,await c.req.json()).revision,p.context));});
  ...
  app.patch(`${base}/patients/:patientId/contact`,async c=>{const input=parse(z.object({phone:z.string().trim().min(3).max(32).regex(/^[+\d ()-]+$/)}).strict(),await c.req.json());return c.json(await service.updateContact(c.get("principal"),parse(id,c.req.param("organizationId")),parse(id,c.req.param("patientId")),input.phone) as any);});
  const reports=`${base}/assessments/:assessmentId/reports`;
  app.post(reports,async c=>{const p=parts(c);return c.json(await service.reports.edit(p.actor,p.org,p.assessment,parse(reportInputSchema,await c.req.json()),p.context));});
  app.patch(`${reports}/:reportId`,async c=>{const p=parts(c);return c.json(await service.reports.edit(p.actor,p.org,p.assessment,parse(reportInputSchema,await c.req.json()),p.context,parse(id,c.req.param("reportId"))));});
  const revision=(c:any)=>parse(z.coerce.number().int().nonnegative(),c.req.query("revision"));
  ...
  app.post(`${reports}/:reportId/files`,async c=>{
    ...
    const input=parse(z.object({revision:z.coerce.number().int().nonnegative(),uploadKey:z.string().min(16).max(128),filename:z.string().min(1).max(255).regex(/^[^\x00-\x1f\x7f/\\]+$/),mediaType:z.enum(["application/pdf","image/jpeg","image/png"]),size:z.coerce.number().int().positive().max(service.config.REPORT_MAX_FILE_BYTES)}),{revision:c.req.header("x-assessment-revision"),uploadKey:c.req.header("x-upload-key"),filename,mediaType:c.req.header("content-type"),size:c.req.header("content-length")});
