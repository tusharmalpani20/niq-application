...
export function createApp(dependencies: AppDependencies) {
  ...
  app.get("/v1/organizations/:organizationId/patients/:patientLocator/activity", zValidator("param", patientParamsSchema, validationFailure), async (context) => context.json({ items: await dependencies.service!.listPatientActivity(context.get("principal"), context.req.valid("param").organizationId, context.req.valid("param").patientLocator) }));
