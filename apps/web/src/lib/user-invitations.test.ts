...
test("submits scoped invitation details and returns development activation links", async () => {
  ...
  expect(await inviteOrganizationUser("org", { email: "test@example.com", role: "OTHER_MEDICAL", facilityIds: [] })).toEqual({ activationToken: "test-token" });
  ...
  expect(JSON.parse(String(request.mock.calls[0]?.[1]?.body))).toEqual({ email: "test@example.com", role: "OTHER_MEDICAL", facilityIds: [] });
});
test("does not report success when the invitation request fails", async () => {
  ...
  await expect(inviteOrganizationUser("org", { email: "test@example.com", role: "OTHER_MEDICAL", facilityIds: [] })).rejects.toThrow("could not be created");
});
