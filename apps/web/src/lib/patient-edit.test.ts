...
import { correctPatientMrn, updatePatient } from "./patient-edit";

const originalFetch = globalThis.fetch;
afterEach(() => { globalThis.fetch = originalFetch; });
const input = { name: "Updated Patient", homeFacilityId: "01ARZ3NDEKTSV4RRFFQ69G5FAV", dateOfBirth: "1999-03-20", gender: "MALE" as const, phone: "9012345678", email: undefined };
...
test("patient edit sends a scoped patch with a mobile number and ISO birth date", async () => {
  ...
  globalThis.fetch = (async (url, options) => {
    ...
    return Response.json({ id: "01ARZ3NDEKTSV4RRFFQ69G5FAW", organizationId: "01ARZ3NDEKTSV4RRFFQ69G5FAX", reference: "PAT-2", homeFacility: { id: "01ARZ3NDEKTSV4RRFFQ69G5FAV", name: "Central" }, dateOfBirth: input.dateOfBirth, gender: input.gender, displayName: input.name, medicalRecordNumber: "MRN-1", createdAt: "2026-09-01", updatedAt: "2026-09-22" });
  }) as typeof fetch;
  ...
  expect(JSON.parse(String(request?.options?.body))).toEqual({ name: input.name, homeFacilityId: input.homeFacilityId, dateOfBirth: "1999-03-20", gender: "MALE", phone: input.phone });
...
test("MRN correction uses the separate reasoned endpoint", async () => {
  ...
  globalThis.fetch = (async (url, options) => {
    request = { url: String(url), options };
    return Response.json({ id: "01ARZ3NDEKTSV4RRFFQ69G5FAW", organizationId: "01ARZ3NDEKTSV4RRFFQ69G5FAX", reference: "PAT-2", homeFacility: { id: input.homeFacilityId, name: "Central" }, dateOfBirth: input.dateOfBirth, gender: input.gender, displayName: input.name, medicalRecordNumber: "MRN-2", createdAt: "2026-09-01", updatedAt: "2026-09-22" });
  }) as typeof fetch;
