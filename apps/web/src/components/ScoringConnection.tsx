...
export function ScoringConnectionPanel({ organizationId, connection, onActivated, onDisconnected, className = "surface admin-detail-card admin-detail-wide", showHeading = true }: {
  ...
  return <section className={className}>
    ...
    {connection && !reconnectRequired ? <>
      <div className="connection-summary"><span className="connection-summary-icon"><Activity aria-hidden="true" /></span><div><strong>{informationState === "error" ? "NIQ Scoring connection needs attention" : "NIQ Scoring connected"}</strong><small>Connected <DateDisplay value={connection.activatedAt} /></small></div><TooltipTrigger><Button className="ml-auto shrink-0" variant="destructive-outline" size="icon" aria-label="Disconnect NIQ Scoring" onPress={() => setConfirmingDisconnect(true)}><Unplug aria-hidden="true" /></Button><Tooltip>Disconnect NIQ Scoring</Tooltip></TooltipTrigger></div>
      ...
      {information && <div className="mt-4 space-y-4" aria-busy={informationState === "loading"}>
        ...
        <div className="flex flex-wrap items-center justify-between gap-2"><div><h3 className="font-semibold">Services and usage</h3><p className="text-sm text-muted-foreground">{usageMonth(information.usage.period)} · UTC</p></div><div className="flex items-center gap-2">{refreshedAt && <span className="text-xs text-muted-foreground">Last loaded <time dateTime={refreshedAt.toISOString()}>{refreshedAt.toLocaleString("en-GB", { day: "numeric", month: "short", hour: "2-digit", minute: "2-digit" })}</time></span>}<TooltipTrigger><Button variant="outline" size="icon" aria-label="Refresh scoring usage" isDisabled={informationState === "loading"} onPress={loadInformation}><RefreshCw className="size-4" aria-hidden="true" /></Button><Tooltip>Refresh scoring usage</Tooltip></TooltipTrigger></div></div>
    ...
    <AlertDialog ariaLabel="Disconnect NIQ Scoring" isOpen={confirmingDisconnect} onOpenChange={(open) => { if (!disconnecting) setConfirmingDisconnect(open); }} isDismissable={!disconnecting}>
      <AlertDialogHeader><AlertDialogMedia><Unplug /></AlertDialogMedia><AlertDialogTitle>Disconnect NIQ Scoring?</AlertDialogTitle><AlertDialogDescription>Are you sure you want to disconnect NIQ Scoring? You will need a new activation token to connect again.</AlertDialogDescription></AlertDialogHeader>
      <AlertDialogFooter><AlertDialogCancel isDisabled={disconnecting}>Cancel</AlertDialogCancel><AlertDialogAction slot={undefined} variant="destructive" isDisabled={disconnecting} onPress={disconnect}>{disconnecting ? "Disconnecting…" : "Disconnect"}</AlertDialogAction></AlertDialogFooter>
    </AlertDialog>
  </section>;
}
