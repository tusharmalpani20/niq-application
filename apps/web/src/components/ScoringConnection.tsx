...
export function ScoringConnectionPanel({ organizationId, connection, onActivated, onDisconnected, className = "surface admin-detail-card admin-detail-wide", showHeading = true }: {
  ...
  return <section className={className}>
    ...
    {connection && !reconnectRequired ? <>
      <div className="connection-summary"><span className="connection-summary-icon"><Activity aria-hidden="true" /></span><div><strong>{informationState === "error" ? "NIQ Scoring connection needs attention" : "NIQ Scoring connected"}</strong><small>Connected <DateDisplay value={connection.activatedAt} /></small></div><Button className="ml-auto" variant="destructive-outline" size="sm" onPress={() => setConfirmingDisconnect(true)}><Unplug aria-hidden="true" />Disconnect</Button></div>
