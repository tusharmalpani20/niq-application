    ...
    <fieldset disabled={isSubmitting} className="form-fields facility-dialog-fields m-0 min-w-0 border-0 grid gap-5">
      <div className="grid gap-4">
        ...
        <div className="grid gap-4 sm:grid-cols-2">
          ...
          <Field data-invalid={!!fieldErrors.medicalRecordNumber || undefined}><FieldLabel htmlFor="patient-mrn" className={patient ? undefined : "required-field-label"}>Medical record number {!patient && <span aria-hidden="true">*</span>}</FieldLabel><Input id="patient-mrn" name={patient ? undefined : "mrn"} defaultValue={patient?.medicalRecordNumber} placeholder="Hospital MRN" maxLength={120} className={patient ? "bg-muted/50" : undefined} readOnly={!!patient} required={!patient} aria-invalid={!!fieldErrors.medicalRecordNumber} aria-describedby={fieldErrors.medicalRecordNumber ? "patient-mrn-error" : undefined} onChange={() => clearFieldError("medicalRecordNumber")} />{patient && <FieldDescription>Only an organization admin can correct this from the patient record.</FieldDescription>}{fieldErrors.medicalRecordNumber && <FieldError id="patient-mrn-error">{fieldErrors.medicalRecordNumber}</FieldError>}</Field>
