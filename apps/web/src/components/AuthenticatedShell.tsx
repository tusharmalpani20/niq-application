...
import { ApiRequestError, getCurrentUser, getOrganization } from "../lib/api";
...
export function AuthenticatedShell({ area }: { area: "platform" | "organization" }) {
  const { resetBranding, updateBranding } = useBranding();
  ...
  useEffect(() => {
    ...
    getCurrentUser()
      .then(async (currentUser) => {
        if (currentUser.platformRole === "NIQ_ADMIN") {
          resetBranding();
        } else {
        ...
        if (active) { setUser(currentUser); setState("authenticated"); }
      })
      .catch((error) => {
        if (!active) return;
        if (error instanceof ApiRequestError && error.response.error.code === "AUTHENTICATION_REQUIRED") {
          resetBranding();
