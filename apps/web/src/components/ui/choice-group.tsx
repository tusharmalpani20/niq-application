...
export function ChoiceGroup({ id, label, options, value, onChange, disabled, required, invalid, describedBy, renderIcon, iconTiles = false }: {
  id: string; label: string; options: ComboboxOption[]; value: string; onChange: (value: string) => void;
  disabled?: boolean; required?: boolean; invalid?: boolean; describedBy?: string; renderIcon?: (optionId: string) => ReactNode; iconTiles?: boolean;
}) {
  return <div id={id} tabIndex={-1}><p className="mb-2 text-sm text-muted-foreground">Select one</p><RadioGroup aria-label={label} aria-describedby={describedBy} value={value} onChange={onChange} isDisabled={disabled} isRequired={required} isInvalid={invalid} className="flex flex-wrap gap-2">
    {options.map(option => <Radio key={option.id} value={option.id} className="assessment-choice flex min-h-12 cursor-pointer items-center gap-2.5 rounded-lg border border-border px-4 py-2.5 text-base outline-none data-selected:border-primary data-selected:bg-primary/5 data-focus-visible:ring-2 data-focus-visible:ring-ring data-disabled:opacity-60">
      {({ isSelected }) => <>{iconTiles ? <span aria-hidden="true" className={`flex size-11 shrink-0 items-center justify-center rounded-xl transition-colors ${isSelected ? "bg-primary text-primary-foreground shadow-sm" : "bg-primary/10 text-primary/70"}`}>{renderIcon?.(option.id)}</span> : <><span className={`flex size-5 shrink-0 items-center justify-center rounded-full border border-current text-brand-ink ${isSelected ? "assessment-selected-indicator" : ""}`} aria-hidden="true">{isSelected && <span className="size-2.5 rounded-full bg-current" />}</span>{renderIcon?.(option.id)}</>}{option.label}</>}
    </Radio>)}
  </RadioGroup></div>;
}
