...
const ALL_FACILITIES_ID = "__all_facilities__";
...
export function FacilityAccessPicker({ idPrefix, facilities, selectedIds, allSelected, allowAll, disabled, invalid = false, errorId, onSelectedIdsChange, onAllChange }: {
  ...
  const options = [
    ...(allowAll && !allSelected ? [{ id: ALL_FACILITIES_ID, label: "All facilities" }] : []),
    ...available.map(facility => ({ id: facility.id, label: label(facility) })),
  ];
  return <div role="group" aria-label="Facility access" className="space-y-3">
    {allSelected && <div role="group" className="flex flex-wrap gap-2" aria-label="Selected facilities"><span className="inline-flex max-w-full items-center gap-1 rounded-full border border-primary/20 bg-primary/5 pl-3 text-sm"><span>All facilities</span><Button variant="ghost" size="icon" className="size-8 shrink-0" aria-label="Remove All facilities" isDisabled={disabled || !allowAll} onPress={() => onAllChange(false)}><X className="size-3.5" aria-hidden="true" /></Button></span></div>}
    ...
    <SearchCombobox key={`${allSelected}-${selectedIds.join(",")}`} id={`${idPrefix}-facility-search`} label="Add facility" placeholder="Search and add facilities…" options={options} value={null} disabled={disabled || options.length === 0} invalid={invalid} describedBy={errorId} onChange={id => {
      if (id === ALL_FACILITIES_ID) { onSelectedIdsChange([]); onAllChange(true); return; }
      if (allSelected) onAllChange(false);
      onSelectedIdsChange(allSelected ? [id] : [...selectedIds, id]);
    }} />
