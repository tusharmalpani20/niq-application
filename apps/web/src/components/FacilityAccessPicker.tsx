...
export function FacilityAccessPicker({ idPrefix, facilities, selectedIds, allSelected, allowAll, disabled, onSelectedIdsChange, onAllChange }: {
  ...
  const available = facilities.filter(facility => !selectedIds.includes(facility.id));
  ...
  return <div role="group" aria-label="Facility access" className="space-y-3">
    {(allowAll || allSelected) && <div className="flex items-center gap-3"><Checkbox id={`${idPrefix}-all-facilities`} aria-label="All facilities" isSelected={allSelected} isDisabled={disabled || !allowAll} onChange={onAllChange} /><label htmlFor={`${idPrefix}-all-facilities`}>All facilities</label></div>}
    {!allSelected && <>
      {selectedIds.length > 0 && <div role="group" className="flex flex-wrap gap-2" aria-label="Selected facilities">{selectedIds.map(id => {
