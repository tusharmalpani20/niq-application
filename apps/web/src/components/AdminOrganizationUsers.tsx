...
export function AdminOrganizationUsers({ users }: { users: OrganizationUser[] }) {
  ...
  const columns: Array<DataTableColumn<OrganizationUser>> = [
    ...
    { id: "role", header: "Role", cell: ({ row }) => roleLabels[row.original.role] },
  ...
  return <section className="surface table-surface">
    <div className="toolbar users-toolbar"><div className="relative w-full sm:w-56"><Input aria-label="Search users" placeholder="Search users…" value={search} onChange={(event) => { setSearch(event.target.value); setPage(1); }} className="pr-9" />{search && <Button type="button" variant="ghost" size="icon-sm" aria-label="Clear search users" className="absolute right-1 top-1/2 -translate-y-1/2 text-muted-foreground" onPress={() => { setSearch(""); setPage(1); }}><X aria-hidden="true" /></Button>}</div></div>
    {users.length === 0 ? <EmptyState title="No users" description="This organization does not have any users yet." /> : filteredUsers.length === 0 ? <EmptyState title="No matching users" description="Try a different name, email, or role." /> : <div className="p-5 pt-0"><DataTable columns={columns} data={visibleUsers} label="Organization users" /></div>}
