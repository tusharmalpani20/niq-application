...
import { manageUserInvitation } from "../lib/user-invitations";

export type InvitationActionTarget = {
  invitation: OrganizationDetails["invitations"][number];
  action: "regenerate" | "revoke";
};

export function TenantInvitationActionDialog({ organizationId, target, onClose, onUpdated }: {
  ...
  async function confirm() {
    ...
    try {
      if (revoking) {
        await manageUserInvitation(organizationId, target.invitation.id, "revoke");
      } else {
        const result = await manageUserInvitation(organizationId, target.invitation.id, "regenerate");
