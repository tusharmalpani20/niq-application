...
export function PatientsPage() {
  ...
  const columns: Array<DataTableColumn<PatientRow>> = [
    { id: "patient", header: "Patient", cell: ({ row }) => <Link className="grid gap-1 text-foreground hover:text-primary hover:underline focus-visible:text-primary" to={`/patients/${row.original.reference}`}><span className="font-normal">{row.original.displayName}</span><span className="text-xs text-muted-foreground">{row.original.reference}</span></Link> },
  ...
  return <>
    ...
    <section className="surface table-surface"><div className="mobile-card-list">{visiblePatients.length ? visiblePatients.map((patient)=><div className="mobile-data-card" key={patient.id}><Link className="min-w-0 flex-1 hover:text-primary focus-visible:text-primary" to={`/patients/${patient.reference}`}><div><div className="font-normal">{patient.displayName}</div><span>{patient.reference} · {patient.age} · {patient.gender}</span></div><span>{patient.facility}</span></Link>{editAction(patient)}</div>) : emptyContent}</div>
