...
export function UsersPage() {
  ...
  const empty = hasFilters ? <EmptyState title="No matches" description="Try changing the search or filters." icon={null} className="min-h-36" /> : canManage ? <div className="flex min-h-40 flex-col items-center justify-center gap-3 text-center"><strong>Invite your first colleague</strong><p className="text-sm text-muted-foreground">Invitations will appear here.</p><Button onPress={() => setInvite(true)}><Plus aria-hidden="true" />Invite a colleague</Button></div> : <EmptyState title="No records to show" icon={null} className="min-h-36" />;
  return <>
