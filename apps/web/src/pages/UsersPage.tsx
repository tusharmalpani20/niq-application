...
export function UsersPage() {
  ...
  const columns: DataTableColumn<OrganizationUser>[] = [
    ...
    { id: "status", header: "Status", cell: ({ row }) => <Badge variant="outline" className={row.original.active && row.original.status === "ACTIVE" ? "border-success/20 bg-success/10 text-success" : "bg-muted text-muted-foreground"}>{row.original.active && row.original.status === "ACTIVE" ? "Enabled" : "Disabled"}</Badge> },
  ];
  if (canManage) columns.push({ id: "actions", header: "Actions", cell: ({ row }) => row.original.userId === currentUser.userId ? <span className="text-xs text-muted-foreground">Your account</span> : <TooltipTrigger><Button size="icon" variant={row.original.active ? "destructive-outline" : "outline"} aria-label={`${row.original.active ? "Disable" : "Enable"} ${row.original.displayName}`} onPress={() => { setActionError(""); setAccessTarget(row.original); }}><Power aria-hidden="true" /></Button><Tooltip>{row.original.active ? "Disable user" : "Enable user"}</Tooltip></TooltipTrigger> });
  ...
  const showEmptyInvite = !loading && !error && total === 0 && !hasFilters && canManage;
  const empty = hasFilters ? <EmptyState title="No matches" description="Try changing the search or filters." icon={null} className="min-h-36" /> : canManage ? <div className="flex min-h-36 items-center justify-center"><Button onPress={() => setInvite(true)}><Plus aria-hidden="true" />Invite a colleague</Button></div> : <EmptyState title="No records to show" icon={null} className="min-h-36" />;
  return <>
    ...
    {error ? <div role="alert" className="surface p-5"><p>{error}</p><Button variant="outline" onPress={() => void load()}>Retry</Button></div> :
      <section className="surface table-surface"><div className="overflow-x-auto p-5">{loading ? <p role="status" className="p-6 text-muted-foreground">Loading users…</p> : total === 0 ? empty : tab === "users" ? <DataTable columns={columns} data={filteredUsers.slice(start, start + 10)} label="Users" emptyContent={empty} /> : <DataTable columns={invitationColumns} data={filteredInvitations.slice(start, start + 10)} label="Pending invitations" emptyContent={empty} />}</div></section>}
