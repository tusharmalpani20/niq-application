        ...
        <div className="form-container">
          ...
          <form className="sign-in-form" onSubmit={handleSubmit}>
            <FieldGroup>
              ...
              <Field><div className="password-row"><FieldLabel htmlFor="password">Password</FieldLabel><Button type="button" variant="link" size="sm" isDisabled>Forgot password?</Button></div><InputGroup className="h-12 overflow-hidden"><InputGroupInput id="password" name="password" type={showPassword ? "text" : "password"} autoComplete="current-password" minLength={8} required /><InputGroupAddon align="inline-end" className="self-stretch border-l bg-accent p-0"><InputGroupButton className="m-0! h-full w-11 rounded-none text-primary hover:bg-secondary" aria-label={showPassword ? "Hide password" : "Show password"} aria-controls="password" aria-pressed={showPassword} onPress={() => setShowPassword((visible) => !visible)} size="icon-sm">{showPassword ? <EyeOff /> : <Eye />}</InputGroupButton></InputGroupAddon></InputGroup></Field>
            </FieldGroup>
