      ...
      <section className="form-panel" aria-labelledby="sign-in-heading">
        <div className="form-container">
          ...
          <form className="sign-in-form" onSubmit={handleSubmit}>
            <FieldGroup>
              <Field><FieldLabel htmlFor="email">Email</FieldLabel><Input className="h-12" id="email" name="email" type="email" autoComplete="username" required placeholder="name@example.com" aria-invalid={Boolean(message)} onChange={() => setMessage(null)} /></Field>
              <Field><FieldLabel htmlFor="password">Password</FieldLabel><InputGroup className="h-12 overflow-hidden" aria-invalid={Boolean(message)}><InputGroupInput id="password" name="password" type={showPassword ? "text" : "password"} autoComplete="current-password" minLength={8} required aria-invalid={Boolean(message)} onChange={() => setMessage(null)} /><InputGroupAddon align="inline-end" className="mr-0! self-stretch border-l bg-accent p-0"><InputGroupButton className="m-0! h-full w-11 rounded-none border-0 bg-clip-border text-primary hover:bg-secondary" aria-label={showPassword ? "Hide password" : "Show password"} aria-controls="password" aria-pressed={showPassword} onPress={() => setShowPassword((visible) => !visible)} size="icon-sm">{showPassword ? <EyeOff /> : <Eye />}</InputGroupButton></InputGroupAddon></InputGroup><div className="flex justify-end"><Button className="px-0" type="button" variant="link" size="sm" isDisabled>Forgot password?</Button></div></Field>
            </FieldGroup>
