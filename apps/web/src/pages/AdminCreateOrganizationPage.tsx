...
export function OrganizationOnboardingForm({ onCancel, onCreated }: { onCancel?: () => void; onCreated?: () => void }) {
  ...
  return <form className="admin-onboarding-form" onSubmit={submit}>
      <section className="admin-form-section"><div><span className="form-step">1</span><h2>Organization</h2><p>The tenant identity shown throughout the platform.</p></div><div className="field-grid"><label>Organization URL name<input name="slug" required pattern="[a-z0-9]+(?:-[a-z0-9]+)*" placeholder="apollo-hospitals" aria-describedby="url-name-help" /><small id="url-name-help" className="field-help">Lowercase letters, numbers and hyphens; used in organization links.</small></label><label>Legal name<input name="legalName" required minLength={2} /></label><label>Display name<input name="displayName" required minLength={2} /></label><label>First administrator email<input name="firstAdminEmail" type="email" required /></label></div></section>
      ...
      <footer className="form-footer">{onCancel ? <button className="btn btn-outline" type="button" onClick={onCancel}>Cancel</button> : <Link className="btn btn-outline" to="/admin/organizations">Cancel</Link>}<button className="btn btn-primary" disabled={busy}>{busy ? "Creating…" : "Create organization and invite admin"}</button></footer>
    </form>;
}
...
export function AdminCreateOrganizationPage() {
  return <>
    ...
    <section className="surface"><OrganizationOnboardingForm /></section>
  </>;
}
