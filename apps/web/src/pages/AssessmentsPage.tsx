...
const initialStatus = (params: URLSearchParams) => {
  ...
  return value === "WORK" || value === "OPEN" || value === "MY_ACTIONS" || value === "SEND_FOR_REVIEW" || value === "CORRECT_DRAFT" || value === "PRIORITY" || value === "COMPLETED_THIS_MONTH" || assessmentStatusSchema.safeParse(value).success ? value! : "all";
};
const matchesStatus = (record: AssessmentSummary, status: string, now: Date) => {
  ...
  if (status === "SEND_FOR_REVIEW" || status === "CORRECT_DRAFT") return record.myAction === status;
