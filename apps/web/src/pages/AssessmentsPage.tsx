...
export function AssessmentsPage() {
  ...
  const columns: Array<DataTableColumn<AssessmentSummary>> = [
    ...
    { id: "patient", header: "Patient", cell: ({ row }) => <div className="grid gap-1"><Link className="text-primary font-semibold" to={`/assessments/${row.original.reference}`}>{row.original.patient.displayName}</Link><span className="text-xs text-muted-foreground">{row.original.patient.reference}</span></div> },
