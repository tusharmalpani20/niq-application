...
export function RiskOverviewCards({ risk, assessments }: { risk: Risk | null; assessments: AssessmentSummary[] }) {
  ...
  return <section className="mt-4 grid gap-4 lg:grid-cols-2" aria-label="Assessment risk and patients needing attention">
    <div className="surface p-5"><h2 className="flex items-center gap-2 font-semibold"><ShieldCheck className="size-5 text-primary" aria-hidden="true" />Final NIQ categories</h2><p className="mt-1 text-xs text-muted-foreground">Each patient’s latest completed assessment</p>
      {risk ? <div className="mt-5 flex flex-wrap items-center gap-6"><div role="img" aria-label={`${categories.map(item => `${item.label}: ${item.count}`).join(", ")}`} className="grid size-36 shrink-0 place-items-center rounded-full" style={{ background: total ? `conic-gradient(var(--primary) 0 ${low}%, #fbbf24 ${low}% ${low + moderate}%, #f43f5e ${low + moderate}% 100%)` : "var(--muted)" }}><div className="grid size-24 place-content-center rounded-full bg-background text-center"><strong className="text-2xl">{total}</strong><span className="text-xs text-muted-foreground">scored patients</span></div></div><ul className="min-w-44 flex-1 space-y-3">{categories.map(item => <li key={item.label} className="flex items-center gap-2 text-sm"><i className={`size-2.5 rounded-full ${item.color}`} /><span className="flex-1">{item.label}</span><strong>{item.count}</strong><span className="w-10 text-right text-xs text-muted-foreground">{total ? Math.round(item.count / total * 100) : 0}%</span></li>)}</ul></div> : <p className="mt-5 text-sm text-muted-foreground">Risk categories are unavailable.</p>}
