...
export function BrandingPage() {
  ...
  function discard() {
    if (!baseline) return;
    setDraft(baseline); setLogo(null); setSaved(false); setMessage(null);
      ...
      <form className="grid gap-5" onSubmit={submit}>
        <fieldset disabled={disabled} className="m-0 grid min-w-0 gap-5 border-0 p-0">
          ...
          <section className="grid gap-3 border-t pt-5">
            ...
            <Field className="max-w-sm"><FieldLabel htmlFor="patientReferencePrefix">Patient prefix</FieldLabel><Input id="patientReferencePrefix" required minLength={2} maxLength={12} pattern="[A-Z][A-Z0-9]{1,11}" value={draft.patientReferencePrefix} onChange={(event) => change("patientReferencePrefix", event.target.value.toUpperCase().replace(/[^A-Z0-9]/g, ""))} /><FieldDescription>Example: {draft.patientReferencePrefix || "PAT"}-1. Applies to new patients only. Existing references stay unchanged.</FieldDescription></Field>
          </section>
        </fieldset>
