    ...
    <div className="assessment-verification-body facility-dialog-fields space-y-5">
      ...
      {finalStep ? <div className="space-y-4">
        ...
        {step?.id === "face_scan" ? scanSession?.state === "COMPLETED" && scanSession.result ? <FaceScanResults session={scanSession}/> : <p className="text-sm text-muted-foreground">{scanStatus === "Done" ? "A completed face scan is saved. Results are loading." : "No completed face scan results are available."}</p>
          ...
          : record.manifest.sections.filter(section => section.id === step?.id).map(section => <div key={section.id} className="grid min-w-0 gap-3">{assessmentFieldGroups(section.fields, record.answers).filter(group => group.some(field => field.kind !== "calculated")).map(group => <dl key={group[0]!.id} data-review-question-group={group[0]!.id} className="grid min-w-0 rounded-lg border border-border bg-muted/20 px-3">{group.filter(field => field.kind !== "calculated" && isAssessmentFieldApplicable(field, record.answers)).map(field => {
