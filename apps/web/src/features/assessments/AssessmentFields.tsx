...
const healthHistoryChoiceFields = new Set(["previous_surgeries", "family_history_cancer"]);
const clinicalGutChoiceFields = new Set(["appetite_status", "bowel_pattern", "stool_frequency"]);
const dietaryChoiceFields = new Set(["dietary_intake"]);
...
export function AssessmentFields({ section, answers, onChange, errors, readOnly, heightSourceDate, onEditContact, patientReference, canEditPatient, canCorrectDob, measurementUnits = metricUnits, onMeasurementUnitsChange }: AssessmentFieldsProps) {
  const renderField = (field: FormField) => {
    ...
    if (field.kind === "multi_select") {
      ...
      const decoratedLongChoices = intakeChoiceFields.has(field.id) || field.id === "co_morbidities" || field.id === "gastrointestinal_symptoms" || field.id === "dietary_symptoms";
      const longChoiceIcon = (optionId: string) => field.id === "co_morbidities" ? <HealthHistoryChoiceIcon type={optionId} /> : field.id === "gastrointestinal_symptoms" ? <ClinicalGutChoiceIcon type={optionId} /> : field.id === "dietary_symptoms" ? <DietarySymptomIcon type={optionId} /> : <MedicationSupplementIcon type={optionId} />;
      ...
      if (allOptions.length <= 6) return <fieldset id={id} key={field.id} tabIndex={-1} aria-describedby={error ? errorId : undefined} aria-invalid={Boolean(error)} className="col-[1/-1] min-w-0 space-y-3">
        ...
        <MultipleChoiceGroup label={label} options={allOptions} value={Array.isArray(value) && !selected.length && explicitNoneFields.has(field.id) ? ["__none__"] : selected} disabled={readOnly} iconTiles={field.id === "tumour_type" || field.id === "current_cancer_treatment"} renderIcon={field.id === "tumour_type" ? optionId => <TumourTypeIcon type={optionId} /> : field.id === "current_cancer_treatment" ? optionId => <TreatmentChoiceIcon type={optionId} /> : undefined} onChange={next => {
      ...
      return <fieldset id={id} key={field.id} tabIndex={-1} disabled={readOnly} aria-describedby={error ? errorId : undefined} aria-invalid={Boolean(error)} className="col-[1/-1] min-w-0 space-y-3">
        ...
        {Array.isArray(value) && <div className="flex flex-wrap items-center gap-2">{selected.length ? selected.map(item => <span key={item} className="inline-flex max-w-full items-center rounded-full border border-primary/20 bg-primary/5 pl-3 text-base">
          {decoratedLongChoices && <span className="mr-2 text-primary" aria-hidden="true">{longChoiceIcon(item)}</span>}
        ...
        </span>) : explicitNoneFields.has(field.id) ? <span className="inline-flex items-center rounded-full border border-primary/20 bg-primary/5 pl-3 text-base">{decoratedLongChoices && <span className="mr-2 text-primary" aria-hidden="true">{longChoiceIcon("__none__")}</span>}None<Button variant="ghost" size="icon" className="ml-1 size-9 rounded-full" isDisabled={readOnly} aria-label={`Remove None from ${field.label}`} onPress={() => onChange(field.id, null)}><X className="size-3.5" /></Button></span> : <span className="text-base text-muted-foreground">No selections</span>}</div>}
        {!readOnly && <div className="max-w-md"><SearchCombobox large key={JSON.stringify(value)} label={`Add ${field.label.toLowerCase()}`} placeholder={`Add ${field.label.toLowerCase()}…`} options={options} value={null} onChange={choice => onChange(field.id, choice === "__none__" ? [] : addAssessmentMultiChoice(selected, choice))} invalid={Boolean(error)} describedBy={error ? errorId : undefined} renderIcon={decoratedLongChoices ? longChoiceIcon : undefined} primaryHighlight={decoratedLongChoices} /></div>}
        {errorMarkup}
      </fieldset>;
    }
    ...
    if (field.id === "height_cm" || field.id === "current_weight_kg" || field.id === "previous_weight_kg") {
      ...
      const isPreviousWeight = field.id === "previous_weight_kg";
      const unit = isHeight ? measurementUnits.height : measurementUnits.weight;
      return <div key={field.id} className="min-w-0 space-y-2">
        <div className="flex min-h-9 flex-wrap items-center justify-between gap-2"><label htmlFor={id} className="block text-base font-medium">{field.label}{isPreviousWeight ? ` (${unit})` : ""}{required}</label><div className="flex items-center gap-1">{!isPreviousWeight && <AssessmentMeasurementUnitControl label={field.label} kind={isHeight ? "height" : "weight"} unit={unit} onUnitChange={next => onMeasurementUnitsChange?.({ ...measurementUnits, [isHeight ? "height" : "weight"]: next })} disabled={typeof value === "string"} />}{reset}</div></div>
        ...
        {isPreviousWeight && <p className="text-xs text-muted-foreground">Uses the unit selected in Personal details</p>}
    ...
    return <div key={field.id} className={`min-w-0 space-y-2 ${field.kind === "select" && (field.options?.length || 0) <= 6 ? "col-[1/-1]" : ""}`}>
      <div className="flex min-h-8 items-center justify-between gap-2"><label htmlFor={id} className="block text-base font-medium">{label}{required}</label>{reset}</div>
      {field.kind === "date" ? <AssessmentDateInput large id={id} label={label} value={typeof value === "string" ? value : ""} disabled={readOnly} required={field.required} invalid={Boolean(error)} describedBy={error ? errorId : undefined} onChange={next => onChange(field.id, next)} /> : field.kind === "select" ? (field.options?.length || 0) <= 6 ? <ChoiceGroup id={id} label={label} options={field.options || []} value={typeof value === "string" ? value : ""} onChange={choice => onChange(field.id, choice)} disabled={readOnly} required={field.required} invalid={Boolean(error)} describedBy={error ? errorId : undefined} iconTiles={diseaseChoiceFields.has(field.id) || treatmentChoiceFields.has(field.id) || healthHistoryChoiceFields.has(field.id) || clinicalGutChoiceFields.has(field.id) || dietaryChoiceFields.has(field.id) || dietaryStatusFields.has(field.id)} renderIcon={diseaseChoiceFields.has(field.id) ? optionId => <DiseaseChoiceIcon type={optionId} /> : treatmentChoiceFields.has(field.id) ? optionId => <TreatmentChoiceIcon type={optionId} /> : healthHistoryChoiceFields.has(field.id) ? optionId => <HealthHistoryChoiceIcon type={optionId} /> : clinicalGutChoiceFields.has(field.id) ? optionId => <ClinicalGutChoiceIcon type={optionId} /> : dietaryChoiceFields.has(field.id) ? optionId => <DietaryIntakeIcon type={optionId} /> : dietaryStatusFields.has(field.id) ? optionId => <DietaryStatusIcon type={optionId} /> : undefined} /> : <SearchCombobox large id={id} label={label} options={field.options || []} value={typeof value === "string" ? value : null} onChange={choice => onChange(field.id, choice)} disabled={readOnly} required={field.required} invalid={Boolean(error)} describedBy={error ? errorId : undefined} primaryHighlight={field.id === "cancer_type"} renderIcon={field.id === "cancer_type" ? optionId => <CancerTypeIcon type={optionId} /> : undefined} onCreate={other ? text => { onChange(field.id, other.option); onChange(other.detail, text); } : undefined} /> : <Input id={id} className="min-h-12 bg-background md:text-base" disabled={readOnly} type={field.kind === "number" ? "number" : "text"} min={field.kind === "number" ? field.min : undefined} step={field.kind === "number" ? field.integer ? 1 : "any" : undefined} inputMode={field.kind === "number" ? "decimal" : undefined} maxLength={ASSESSMENT_ANSWER_TEXT_LIMIT} aria-required={field.required} aria-invalid={Boolean(error)} aria-describedby={error ? errorId : undefined} value={typeof value === "string" || typeof value === "number" ? value : ""} onChange={event => {
