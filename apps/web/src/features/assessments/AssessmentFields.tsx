  ...
  const renderField = (field: FormField) => {
    ...
    if (field.id === "height_cm" || field.id === "current_weight_kg" || field.id === "previous_weight_kg") {
      ...
      return <div key={field.id} className="min-w-0 space-y-2">
        <div className="flex min-h-9 flex-wrap items-center justify-between gap-2"><label htmlFor={id} className="block text-base font-medium">{field.label}{isPreviousWeight ? ` (${unit})` : ""}{required}</label><div className="flex items-center gap-1">{reset}{!isPreviousWeight && <AssessmentMeasurementUnitControl label={field.label} kind={isHeight ? "height" : "weight"} unit={unit} onUnitChange={next => onMeasurementUnitsChange?.({ ...measurementUnits, [isHeight ? "height" : "weight"]: next })} disabled={typeof value === "string"} />}</div></div>
