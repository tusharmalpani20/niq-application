  ...
  const renderField = (field: FormField) => {
    ...
    if (field.kind === "multi_select") {
      ...
      if (allOptions.length <= 6) return <fieldset id={id} key={field.id} tabIndex={-1} aria-describedby={error ? errorId : undefined} aria-invalid={Boolean(error)} className="col-[1/-1] min-w-0 space-y-3">
        ...
        <MultipleChoiceGroup label={label} options={allOptions} value={Array.isArray(value) && !selected.length && explicitNoneFields.has(field.id) ? ["__none__"] : selected} disabled={readOnly} iconTiles={field.id === "tumour_type"} renderIcon={field.id === "tumour_type" ? optionId => <TumourTypeIcon type={optionId} /> : undefined} onChange={next => {
