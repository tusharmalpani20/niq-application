...
const icons = { personal_details: UserRound, face_scan: ScanFace, disease_status: Cross, treatment: Stethoscope, health_history: History, clinical_gut_health: HeartPulse, dietary_details: Utensils, reports: FileText, review: ListChecks };
...
export function AssessmentSectionNavigation({ tabs, selected, coverage, scores = {}, scanStatus = "Pending", attachmentCounts, disabled, onSelect }: {
  ...
  const attachmentSummary = `${attachmentCounts.reports} ${attachmentCounts.reports === 1 ? "report" : "reports"} · ${attachmentCounts.files} ${attachmentCounts.files === 1 ? "file" : "files"}`;
  const items = tabs.map(tab => {
    ...
    return <Button key={tab.id} variant="ghost" aria-label={tab.id === "face_scan" ? `${tab.title}: ${scanStatus}` : tab.id === "reports" ? `${tab.title}: ${attachmentSummary}` : tab.title} className={`${tab.id === "reports" ? "h-14" : "h-12"} w-full justify-start gap-2.5 whitespace-nowrap rounded-lg px-3 py-2.5 text-left text-sm ${selected === tab.id ? "assessment-active-step text-brand-ink font-semibold" : "text-muted-foreground"}`} aria-current={selected === tab.id ? "step" : undefined} isDisabled={disabled} onPress={() => { onSelect(tab.id); }}><Icon className="size-4 shrink-0" aria-hidden="true" />{tab.id === "reports" ? <span className="min-w-0 flex-1 text-left"><span className="block truncate">{tab.title}</span><span className="block truncate text-[11px] font-normal tabular-nums">{attachmentSummary}</span></span> : <span className="min-w-0 flex-1 truncate">{shortLabels[tab.id] ?? tab.title}</span>}{tab.id === "face_scan" ? <span title={scanStatus} className="max-w-16 shrink-0 truncate text-[11px]">{scanStatus}</span> : completion?.percent !== null && completion?.percent !== undefined && <span title={scores[tab.id]} className="max-w-16 shrink-0 truncate text-[11px] tabular-nums">{scores[tab.id] ?? `${completion.percent}%`}</span>}</Button>;
  });
          ...
          <SelectList className="min-h-0 max-h-72">
            {tabs.map(tab => {
              ...
              return <SelectItem key={tab.id} id={tab.id} textValue={tab.title} className="min-h-11">
                ...
                {tab.id === "face_scan" ? <span className="text-xs text-muted-foreground">{scanStatus}</span> : tab.id === "reports" ? <span className="text-xs tabular-nums text-muted-foreground">{attachmentSummary}</span> : completion?.percent != null && <span className="text-xs tabular-nums text-muted-foreground">{scores[tab.id] ?? `${completion.percent}%`}</span>}
              </SelectItem>;
            })}
          </SelectList>
        </SelectPopover>
