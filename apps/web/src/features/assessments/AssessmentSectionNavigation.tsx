...
export function AssessmentSectionNavigation({ tabs, selected, coverage, disabled, onSelect }: {
  ...
  const items = tabs.map(tab => {
    ...
    return <Button key={tab.id} variant="ghost" aria-label={tab.title} className={`h-auto min-h-11 w-full justify-start gap-2.5 whitespace-normal rounded-lg px-3 py-2.5 text-left text-sm ${selected === tab.id ? "bg-primary/10 text-brand-ink font-semibold" : "text-muted-foreground"}`} aria-current={selected === tab.id ? "step" : undefined} isDisabled={disabled} onPress={() => { onSelect(tab.id); }}><Icon className="size-4 shrink-0" aria-hidden="true" /><span className="flex-1">{tab.title}</span>{completion?.percent !== null && completion?.percent !== undefined && <span className="text-[11px] tabular-nums">{completion.percent}%</span>}</Button>;
  });
          ...
          <SelectList className="min-h-0 max-h-72">
            {tabs.map(tab => {
              ...
              return <SelectItem key={tab.id} id={tab.id} textValue={tab.title} className="min-h-11">
                ...
                {completion?.percent != null && <span className="text-xs tabular-nums text-muted-foreground">{completion.percent}%</span>}
              </SelectItem>;
            })}
          </SelectList>
        </SelectPopover>
