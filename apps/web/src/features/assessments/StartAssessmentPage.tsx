...
function ScopedStartAssessmentPage({ user }: { user: AuthenticatedUser }) {
  ...
  return <div className="assessment-workflow @container">
    ...
    <section className="overflow-hidden rounded-xl border border-border bg-card" aria-label="Patient selection">
      ...
      {!loaded || loadFailed ? <>
        {!loaded ? <div className="p-5 text-sm text-muted-foreground">Loading patient information…</div> : <div className="p-5"><Button variant="outline" onPress={() => setLoadKey(key => key + 1)}>Retry loading</Button></div>}
        <footer className="border-t border-border px-5 py-3"><Button variant="outline" onPress={() => navigate(exitPath, { replace: true })}>Exit</Button></footer>
      </> : mode === "create" ?
        <PatientForm organizationId={user.organizationId} facilities={facilities} initialName={newPatientName} initialFacilityId={facility || undefined} submitLabel="Create patient & continue" cancelLabel="Choose existing patient" onCancel={() => setMode("select")} onExit={() => navigate(exitPath, { replace: true })} onSaved={patient => { setPatients(current => [patient, ...current]); void start(patient); }} />
        : <>
          <div className="clinical-form grid gap-5 p-5">
            {committed && selected ? <div><p className="text-sm text-muted-foreground">Patient for this assessment</p><p className="mt-1 font-medium">{selected.displayName} · {selected.reference}</p><p className="text-sm text-muted-foreground">{selected.homeFacility?.name ?? "No facility"}</p></div> : <>
              ...
              <Field><FieldLabel htmlFor="assessment-patient">Patient</FieldLabel><SearchCombobox id="assessment-patient" label="Patient" value={selected?.id ?? null} disabled={busy} placeholder="Search name, patient reference or MRN" options={filtered.map(patient => ({ id: patient.id, label: `${patient.displayName} · ${patient.reference}${patient.medicalRecordNumber ? ` · MRN ${patient.medicalRecordNumber}` : ""} · ${patient.homeFacility?.name ?? "No facility"}` }))} onChange={id => setSelected(filtered.find(patient => patient.id === id) ?? null)} onCreate={canCreatePatient ? name => { setNewPatientName(name); setMode("create"); } : undefined} createWhenNoMatches createOptionLabel={name => `No patient found · Create “${name}”`} /></Field>
              {canCreatePatient && <div className="flex flex-wrap items-center gap-2 rounded-xl border border-primary/25 bg-primary/5 px-4 py-3"><span className="text-sm font-medium">Can't find the patient?</span><Button variant="outline" className="border-primary/50 bg-background text-brand-ink hover:border-primary hover:bg-primary/10" onPress={() => { setNewPatientName(""); setMode("create"); }}><Plus aria-hidden="true" data-icon="inline-start" />Create new patient</Button></div>}
            </>}
          </div>
          <footer className="flex flex-wrap items-center justify-between gap-3 border-t border-border px-5 py-3">
            <Button variant="outline" isDisabled={busy} onPress={() => navigate(exitPath, { replace: true })}>Exit</Button>
