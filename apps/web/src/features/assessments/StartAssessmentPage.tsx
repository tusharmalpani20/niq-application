  ...
  return <div className="assessment-workflow @container">
    ...
    <section className="overflow-hidden rounded-xl border border-border bg-card" aria-label="Patient selection">
      ...
      {!loaded || loadFailed ? <>
        ...
        <PatientForm organizationId={user.organizationId} facilities={facilities} submitLabel="Create patient & continue" cancelLabel="Choose existing patient" onCancel={() => setMode("select")} onExit={() => navigate(exitPath, { replace: true })} onSaved={patient => { setPatients(current => [patient, ...current]); void start(patient); }} />
        : <>
