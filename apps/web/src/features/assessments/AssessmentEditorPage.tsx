...
function AssessmentEditor({ organizationId, userId, assessmentId, role }: { organizationId: string; userId: string; assessmentId: string; role: MembershipRole }) {
  ...
  const scanCancelableAfterScore = record?.status === "SCORED" && !clinical.error && clinical.review?.allowedActions.some(action => action === "SEND" || action === "RESEND");
  ...
  return <div className="assessment-workflow @container">
    ...
    {(record.result !== null || clinical.review?.state === "RETURNED" || !!clinical.error) && <ClinicalReviewPanel organizationId={organizationId} assessmentId={internalId} review={clinical.review} attestations={record.attestations} error={clinical.error} loading={clinical.loading} blocked={dirty || reportDirty || busy || reportBusy || scanBusy || contactOpen || conflict} onRefresh={clinical.refresh} onChanged={async () => { await reload(); await clinical.refresh(); }} onBusyChange={setReviewBusy} />}
    {record.result !== null && !showScoredAnswers && <AssessmentScoreReview canReview={hasPermission(role, "scores.review") && clinical.review?.canAdjustScores === true && !reviewBusy && !clinical.loading && !clinical.error} onSaved={clinical.refresh} scanStatus={scanStatus} record={record} organizationId={organizationId} renderScan={active => <AssessmentFaceScan organizationId={organizationId} record={record} active={active} disabled={!hasPermission(role, "scans.perform") || !(editable || scanCancelableAfterScore) || busy || reportBusy || reviewBusy || conflict} beforeStart={persist} onBusyChange={setScanBusy} onStatusChange={setScanStatus}/>} reportsContent={<AssessmentReports organizationId={organizationId} assessmentId={internalId} revision={record.revision} reports={record.reports} limits={record.reportLimits} readOnly={!hasPermission(role, "reports.manage") || !editable || busy || conflict} onChanged={refreshReports} onBusyChange={setReportBusy} onDirtyChange={setReportDirty} />} />}
    ...
    <div className={`grid min-w-0 border-x border-border bg-card @min-[48rem]:grid-cols-[250px_minmax(0,1fr)] ${separatedEditor ? "rounded-t-xl border-t" : ""}`}>
      ...
      <div className="min-w-0 p-4 sm:p-6"><div className="mb-5 flex flex-wrap items-center justify-between gap-2"><h2 id="assessment-section-heading" tabIndex={-1} className="flex scroll-mt-40 items-center gap-2 text-xl font-semibold outline-none">{sectionId === "face_scan" && <ScanFace className="size-5 shrink-0 text-primary" aria-hidden="true" />}{tabs[index]?.title}</h2>{sectionId === "reports" && <span className="text-xs text-muted-foreground tabular-nums">{attachmentCounts.files} {attachmentCounts.files === 1 ? "file" : "files"} uploaded</span>}{sectionCoverage && <span className="text-xs text-muted-foreground">{sectionCoverage.answered}/{sectionCoverage.total} answered · {sectionCoverage.percent ?? 0}%</span>}</div>
        {(record.result === null || showScoredAnswers) && <AssessmentFaceScan organizationId={organizationId} record={{ ...record, answers }} active={sectionId === "face_scan"} disabled={!hasPermission(role, "scans.perform") || !(editable || scanCancelableAfterScore) || busy || reportBusy || reviewBusy || conflict} beforeStart={persist} onMissingInputs={showMissingScanInputs} onBusyChange={setScanBusy} onStatusChange={setScanStatus} onSessionChange={setScanSession}/>}
    ...
    <footer className="sticky bottom-0 z-10 flex flex-wrap items-center justify-between gap-3 rounded-b-xl border border-border bg-card px-4 py-3 sm:px-5">
      ...
      <div className="flex flex-wrap gap-2">{editable && <Button variant="outline" className="h-10" isDisabled={locked || !dirty} onPress={() => { void persist(); }}>{busy ? "Saving…" : "Save"}</Button>}{index < tabs.length - 1 ? <Button className="h-10" isDisabled={locked} onPress={() => { void selectSection(tabs[index + 1]!.id, true); }}>Continue<ArrowRight aria-hidden="true"/></Button> : editable && hasPermission(role, "assessments.submit") && <Button className="h-10" isDisabled={locked || reportDirty || progress.percent !== 100} onPress={() => { void openVerification(); }}>Submit and request score<ArrowRight aria-hidden="true"/></Button>}</div>
    </footer>
    </div>
