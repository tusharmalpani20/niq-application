...
async function harness(run:(ctx:{click:(name:string)=>Promise<void>;posts:any[]})=>Promise<void>, entries:ScoreReviewEntry[] = [], conflict=false) {
 ...
 for(const key of ["FocusEvent","HTMLElement","SVGElement","Element","Node","NodeFilter","DocumentFragment","HTMLButtonElement","HTMLInputElement","MutationObserver"]) values[key]=(dom.window as any)[key];
 const posts:any[]=[];
