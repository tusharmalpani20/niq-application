...
export function AssessmentScoreReview({ record, organizationId, renderScan, reportsContent, onDirtyChange, onSaved, canReview = true, scanStatus = "Face scan unavailable" }: {
  ...
  return <section className="my-5 min-w-0 rounded-xl border border-border bg-card p-4 sm:p-6" aria-label="Assessment score review">
    ...
    {!!data?.entries.length && data && <details className="mt-4 border-t border-border pt-4"><summary className="min-h-11 cursor-pointer font-medium">Score history</summary><ol className="divide-y divide-border">{[...data.entries].reverse().map(entry => <li key={entry.id} className="py-3 text-sm"><p className="font-medium">{entry.targetType === "scan" ? "Face scan score" : entry.targetType === "overall" ? "Overall score" : entry.targetType === "section" ? sections.find(section => section.id === entry.targetId)?.title : result.components.find(item => item.id === entry.targetId)?.label} · {points(entry.previousPoints)} → {entry.points === null ? "Restored" : points(entry.points)}</p><p className="mt-1">Changed by: {entry.actorName || "Unknown user"}</p><p className="text-muted-foreground"><time dateTime={entry.createdAt}>{new Date(entry.createdAt).toLocaleString("en-GB")}</time></p><p className="mt-1 whitespace-pre-wrap break-words text-muted-foreground">Reason: {entry.reason || "Not provided"}</p></li>)}</ol></details>}
  </section>;
}
